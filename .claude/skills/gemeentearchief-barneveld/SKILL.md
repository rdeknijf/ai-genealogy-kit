...
description: |
  ...
  `openarchieven-api` for 95% of queries. Relevant for Peters research
...
python scripts/openarchieven_search.py "Peters" --place Barneveld

# Explicit Gelders Archief filter (BS/DTB only)
python scripts/openarchieven_search.py "Peters" --archive gld --place Barneveld

# Verified Peters hit count: 4 BS Overlijden records
# (Jan Riksen/Rikzen Peters, Peter Peters)
```
...
- **Peters line (Nijmegen-Barneveld family)** — RQ-001 brick wall.
  Barneveld Peters are BS Overlijden in GA → use `gld --place Barneveld`.
