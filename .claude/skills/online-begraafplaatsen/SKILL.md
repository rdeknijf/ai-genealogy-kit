...
```bash
curl -s 'https://api.online-begraafplaatsen.nl/bgp/V1/PERSselect?term=Jansen'
```
...
```json
[
  {"label": "Jansen de, Jacob (1924-1968)", "value": 1194484},
  {"label": "Jansen de, Jan (-1970)", "value": 1061177}
]
```
...
curl -s -b "$COOKIE_JAR" \
  ...
  -X POST \
  -d "CSRFToken=$CSRF&Achternaam=Jansen&tussenvoegsel=de&voornaam=Jan&Select1=0&submitbutton=Zoek"
```
...
- **Surname goes WITHOUT prefix** — "Jansen" not "de Vries". The tussenvoegsel
  has its own field.
