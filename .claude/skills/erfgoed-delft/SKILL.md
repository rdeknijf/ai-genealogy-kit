...
description: |
  ...
  Schipluiden. Part of Erfgoed Delft. 185+ Jansen person records found,
...
# Search for all Jansen records
curl -s -b /tmp/delft.jar -L \
  'https://zoeken.stadsarchiefdelft.nl/zoeken/groep=Personen/Achternaam=Jansen/aantalpp=50/'

# Search for Jansen with prefix "de"
curl -s -b /tmp/delft.jar -L \
  'https://zoeken.stadsarchiefdelft.nl/zoeken/groep=Personen/Tussenvoegsel=de/Achternaam=Jansen/aantalpp=50/'
...
```bash
curl -s -b /tmp/delft.jar -L \
  'https://zoeken.stadsarchiefdelft.nl/zoeken/groep=Personen/Achternaam=Jansen/aantalpp=50/pagina=1/'
```
...
curl -s -b /tmp/delft.jar -L \
  'https://zoeken.stadsarchiefdelft.nl/zoeken/groep=Personen/Achternaam=Jansen/aantalpp=50/?filters%5BSoort%20Akte%5D%5B%5D=Burgerlijke%20Stand'

# Filter by Bevolking (population register)
curl -s -b /tmp/delft.jar -L \
  'https://zoeken.stadsarchiefdelft.nl/zoeken/groep=Personen/Achternaam=Jansen/aantalpp=50/?filters%5BSoort%20Akte%5D%5B%5D=Bevolking'
```
...
field combinations. The surname `Jansen` will also match `de Vries`
and `van der Jansen`.
