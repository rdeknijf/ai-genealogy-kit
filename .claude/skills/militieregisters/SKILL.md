...
|-----------|-------------|---------|
| `name` | Surname (required) | `Jansen`, `de+Jansen` |
...
```bash
# All militieregisters for "Jansen" (46 results)
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&sourcetype=Militieregisters&number_show=25&lang=nl"

# Filter to Barneveld
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&sourcetype=Militieregisters&eventplace=Barneveld&number_show=25&lang=nl"
...
```json
{
  "personname": "Adrianus Wilhelm Jansen",
