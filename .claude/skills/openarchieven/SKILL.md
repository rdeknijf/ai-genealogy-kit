...
- Simple surname: `name=Jansen`
- Full name: `name=Jan+de+Jansen`
- Couple search (finds marriage records): `name=Jan+de+Jansen+%26+Maria` (use `&` encoded as `%26`)
- With year: `name=Jansen+1850`
- With year range: `name=Jansen+1800-1850`
...
```bash
# Search for surname "Jansen", first 20 results
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&lang=en&number_show=20"

# Search for marriage records of "Jansen" in Apeldoorn
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&sourcetype=BS+Huwelijk&eventplace=Apeldoorn&lang=en&number_show=20"

# Search for a couple (finds records linking both names)
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jan+de+Jansen+%26+Maria&lang=en&number_show=10"

# Sort by date descending, Netherlands only
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&country_code=nl&sort=-4&lang=en&number_show=20"
```
...
```json
{
  "query": { "name": "Jansen", "number_show": 10, ... },
  "response": {
    "number_found": 7756,
    "start": 0,
    "docs": [
...
```bash
# Page 1: results 0-99
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&number_show=100&start=0&lang=en"

# Page 2: results 100-199
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jansen&number_show=100&start=100&lang=en"
```
