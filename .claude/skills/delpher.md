...
**Example -- search for familieberichten mentioning "de Vries" in newspapers, sorted by date:**

```bash
curl -s 'https://www.delpher.nl/nl/api/results?query=%22de+Jansen%22&coll=ddd&page=1&maxperpage=50&facets%5Btype%5D%5B%5D=familiebericht&sortfield=date&order=asc'
```
...
```bash
curl -s 'https://jsru.kb.nl/sru/sru?version=1.2&operation=searchRetrieve&x-collection=DDD_artikel&query=%22de+Jansen%22&recordSchema=ddd&maximumRecords=20'
```
...
Use double quotes around names for exact phrase matching, e.g. `"de Vries" Bennekom`.
...
- **Exact name matching**: `%22Jan+de+Jansen%22` (URL-encoded quotes)
- **Name + place**: `%22de+Jansen%22+Bennekom` finds the name near the place
