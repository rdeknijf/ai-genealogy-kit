...
```bash
# Search for surname "Jansen" in Curacao Soldijboeken
curl -s -X POST "https://www.westindischecompagnie.info/zoek1.php" \
  -d "achternaam=Jansen&voornaam=&herkomst=" \
  | iconv -f windows-1250 -t utf-8
```
...
```bash
# Search for surname "Jansen" in WIC Personnel 1621-1675
curl -s -X POST "https://www.westindischecompagnie.info/wic1personeel.php" \
  -d "achternaam=Jansen&voornaam=&geboorteplaats=" \
  | iconv -f windows-1250 -t utf-8
```
