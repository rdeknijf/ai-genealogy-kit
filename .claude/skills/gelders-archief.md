...
- `name` -- search query (supports multiple names separated by `&` for
  combined record search, e.g., `Jacob de Vries & Geesje van den Berg`)
...
```bash
curl -s "https://api.openarchieven.nl/1.1/records/search.json?name=Jacob+de+Jansen&archive_code=gld&eventplace=Bennekom&sourcetype=BS+Geboorte&number_show=20&lang=nl"
```
...
```
name=Jacob+de+Jansen+%26+Geesje+van+den+Hul
```
